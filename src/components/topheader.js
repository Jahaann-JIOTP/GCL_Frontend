...
const TopHeader = () => {
  ...
  const fetchAlarms = async () => {
    ...
    try {
      const response = await axios.get("https://www.gclapi.jiotp.com/gcl_backend/bell.php");
  ...
  {alarms.length > 0 ? (
    alarms.map((alarm, index) => (
      <li key={index} className="py-2 border-b flex items-center">
        ...
        <div className="text-right text-[12px] ml-auto">{alarm.time}</div>
      </li>
    ))
  ) : (
    <li className="text-center py-4 text-gray-500">No alarms available.</li>
  )}
  {/* Details link */}
  <li
