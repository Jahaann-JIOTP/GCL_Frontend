...
function CustomTrend() {
  ...
  useEffect(() => {
    if (startDate && endDate && selectedMeter.length > 0 && selectedParameter) {
      const apiUrl = `https://www.gclapi.jiotp.com/gcl_backend/line_api.php?start_date=${startDate}&end_date=${endDate}&meterId=${selectedMeter
  ...
  return (
    ...
    //   </p>
    // </div>
    <div className="flex-shrink-0 w-full p-6 h-[85vh] rounded-[8px] bg-[#f2f2f2] border-2 border-[grey] border-t-[4px] border-t-[#1d5999] opacity-75">
