...
const AlarmTable = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      axios
        .get("https://www.gclapi.jiotp.com/gcl_backend/comparison.php") // Update to your backend API URL
        .then((response) => {
      ...
      <div className="overflow-x-auto h-[75%]">
        <table className="min-w-full table-auto border-collapse border border-gray-300">
          <thead className="bg-blue-50 border-b border-gray-300">
            <tr>
              <th className="p-2 text-left text-sm font-semibold text-gray-700">State</th>
              <th className="p-2 text-left text-sm font-semibold text-gray-700">Source</th>
              <th className="p-2 text-left text-sm font-semibold text-gray-700">Status</th>
              <th className="p-2 text-left text-sm font-semibold text-gray-700">Last Occurrence</th>
              <th className="p-2 text-left text-sm font-semibold text-gray-700">Occurrences</th>
            </tr>
          </thead>
          <tbody>
              ...
              return (
                ...
                >
                  <td className="p-2 text-sm flex items-center relative">
                    <div className={`h-full w-1 absolute top-0 left-0 ${color}`}></div>
                    {image && (
                      <img
                        src={image}
